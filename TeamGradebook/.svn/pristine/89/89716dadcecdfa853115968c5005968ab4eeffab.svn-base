...
public class TeamGradebook {
	...
	private String handleGetAbsences(String studentName) {
		return Integer.toString(getStudentByName(studentName).getAbsence());
	}
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		for (int i = 0; i < memberNames.size(); i++) {
			if (this.getStudentByName(memberNames.get(i)) == null) {
				Student newStudent = new Student(memberNames.get(i));
				this.students.add(newStudent);
				}
		}

		for (int i = 0; i < memberNames.size(); i++) {
			Student student = getStudentByName(memberNames.get(i));
			newTeam.addStudentToTeam(teamName, student);
		}
		teams.add(newTeam);
		return "ok";
	}
		...
		for (int i = 0; i < teams.size(); i++) {
			if (teams.get(i).getName().equals(teamName)) {
				for (int j = 0; j < teams.get(i).getMembers().size(); j++) {
					teams.get(i).getMembers().get(j).addGrade(grade);
				}
				teams.get(i).addGrade(grade);
			}
		}
	...
	private String handleGetAverage(String studentName) {
		// DONE: Your code here
		return Long.toString(Math.round(getStudentByName(studentName).getAverageGrade()));
	}
	...
	private String handleGetBestTeam() {
		...
		String team = "";
		for (int i = 0; i < teams.size(); i++) {
			if (teams.get(i).getAverageGrade() > highest) {
				team = teams.get(i).getName();
				highest = teams.get(i).getAverageGrade();
			}
		}
		return team;
	}

...
public class MapAnd2DArrayHomework {
	...
	public static HashMap<String, ArrayList<Integer>> reverseHashmap(HashMap<Integer, String> initialMap) {
		...
		for (int i : initialMap.keySet()) {
			if (map.containsKey(initialMap.get(i))) {
				...
				map.put(initialMap.get(i), output);
				output.add(i);
			}
		}
	...
	public static String getTemperatureDropCity(int[] temps, String[] tempCity) {
		...
		for (int i = 0; i < tempCity.length; i++) {
			if (map.containsKey(tempCity[i])) {
				map.get(tempCity[i]).add(temps[i]);
			} else {
				ArrayList<Integer> output = new ArrayList<Integer>();
				map.put(tempCity[i], output);
				output.add(temps[i]);
			}
		}
		for (String current : map.keySet()) {
			for (int i = 0; i < map.get(current).size() - 1; i++) {
				if (map.get(current).get(i) - map.get(current).get(i + 1) > 0) {
					return current;
				}
			}
		}
	...
	public static int getNumberOfCoursesToTake(HashMap<String, String> courseMap, String course) {
		...
		if (courseMap.get(course) == course) {
			return -1;
		}

		else {
			String classss = course;
			ArrayList<String> output = new ArrayList<String>();
			while (courseMap.get(classss) != "" && classss != courseMap.get(courseMap.get(classss))) {
				if (output.contains(classss)) {
					return -1;
				}
				output.add(classss);
				count += 1;
				classss = courseMap.get(classss);
			}
		}
		return count;
	}
	...
	public static char[][] distanceArray(int row, int col, int distance) {
		...
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				if (Math.abs(initX - i) + Math.abs(initY - j) > distance) {
					result[i][j] = '.';
				} else {
					result[i][j] = 'x';
				}
	...
	public static int ballRestElevation(int[][] map) {
		...
		int initY = 1;
		int finalNumber = 0;
		int dirOfBiggestDifference = 0;
		for (int i = 1; i<map.length-1;i++){
			for(int j = 1; j<map[i].length-1;j++){
				if(map[i-1][j]-map[i][j]>map[i][j-1]-map[i][j]){
					initX = i-1;
					initY = j;
				}else if(map[i-1][j]-map[i][j]<map[i][j-1]-map[i][j]){
					dirOfBiggestDifference = map[i][j-1];
					initX = i;
					initY = j-1;
				}if(map[i+1][j]-map[i][j]>map[i][j+1]-map[i][j]){
					dirOfBiggestDifference = map[i+1][j];
					initX = i+1;
					initY = j;
				}else if(map[i+1][j]-map[i][j]<map[i][j+1]-map[i][j]){
					dirOfBiggestDifference = map[i][j+1];

...
public class CrazyEightsCommandTests {
	...
	/*
	 * New tests that have been implemented
	 */
	
	// Tests whether the input is too long
	@Test 
	public void testInputSize(){
		...
		game.handleCommand("start-game 2 testDeck false");
		String[] result = game.handleCommand("play-card JCC").split("\n");
		compareStringArrayResults(expected, result);
	}
	
	// Tests the "start-game" command and makes sure that it doesn't print the player and his deck properties.
	@Test 
	public void testInvalidStartGame() {
		String[] expected = new String[] {
				"Unknown command star-game"
		};
		String[] result = game.handleCommand("star-game 2 testDeck false").split("\n");
		compareStringArrayResults(expected, result);
	}
	
	
	
	// Tests whether the drawing deck fills up from the playing deck.
	// This is done by getting the new cards to the point where it has
	// to be filled up with discard cards in the file partGame4 and 
	// checking whether drawing a card from the pile afterwards produces
	// an output
	@Test
	public void testPartGame() {
		exit.expectSystemExitWithStatus(-300); 
		// detects if you exit earily.  DO NOT exit with status -300.
		testGame("partGame4.txt");
		String[] result = game.handleCommand("draw-card").split("\n");
